...
async def play(ctx,song):
    if not ctx.message.author.voice:
        await ctx.send("{} Intră pe voice dacă vrei să-ți cânt!".format(ctx.message.author.name))
        return
    else:
        ...
        try :
            ...
            if song in song_list:
                ...
                await ctx.send('**Dedicație: **' + song)
            else :
                await ctx.send("Nu știu să cânt asta!")
        except :
            await ctx.send("Am luat pauză barosane!")

@bot.command()
async def list(ctx):
    await ctx.send("Cântece pentru dedicație")
...
@bot.command(brief='Ca să plece botul')
async def scram(ctx):
...
@bot.command(brief='Oprirea dedicației')
async def pause(ctx):
    ...
    if voice.is_playing():
        ...
        await ctx.send("Sunt pe pauză momentan!")
...
async def resume(ctx):
    ...
    if voice.is_paused():
        voice.resume()
        await ctx.send("Hai să continuăm!")
    else:
        await ctx.send("Deja cânt altceva!")
